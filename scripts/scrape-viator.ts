...
const CITIES = [
  { name: "Tokyo", slug: "tokyo", region: "Kanto", viatorPath: "Tokyo/d334" },
  { name: "Kyoto", slug: "kyoto", region: "Kansai", viatorPath: "Kyoto/d332" },
  { name: "Osaka", slug: "osaka", region: "Kansai", viatorPath: "Osaka/d4821" },
  {
    name: "Hiroshima",
    slug: "hiroshima",
    region: "Chugoku",
    viatorPath: "Hiroshima/d4420",
  },
  { name: "Nara", slug: "nara", region: "Kansai", viatorPath: "Nara/d4822" },
  {
    name: "Hakone",
    slug: "hakone",
    region: "Kanto",
    viatorPath: "Hakone/d5041",
  },
];
...
async function scrapeCity(
  ...
  try {
    while (results.length < maxItems) {
      const url = `https://www.viator.com/${city.viatorPath}-tours/allthingstodo?page=${pageNum}`;
...
function parsePrice(priceStr: string): { amount: number; currency: string; display: string } {
...
function parseDuration(str: string): { hours: number; display: string } {
  ...
  let hours = 0;
  if (daysMatch) hours = parseInt(daysMatch[1]) * 8;
...
async function main() {
  console.log("🐻 Starting Viator scraper...\n");

  const allResults: Record<string, unknown>[] = [];
  const outputPath = join(process.cwd(), "data", "viator-raw.json");
  ...
  for (const city of CITIES) {
    ...
    for (const item of raw) {
      ...
      allResults.push({
        id: `viator-${city.slug}-${slug}`,
        slug: `${city.slug}-${slug}`,
        title: item.title,
        description: item.description || `Experience ${item.title} in ${city.name}, Japan.`,
        shortDescription:
